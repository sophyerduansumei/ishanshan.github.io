...
以上，可用工具不少，比如看板工具 [Trello](https://trello.com/) 、协作工具 [Spark](http://spark.yi-gather.com/home?redirect=http%3A%2F%2Fspark.yi-gather.com%2Fdesk) 。但个人偏爱 [WorkFlowy](https://workflowy.com/) ，无它，缘分而已。
...
日常临时接到或忽然想起，但又难以立马想到安排在哪天，或实在太零碎不适合写进日报的事情，我会及时记录在碎片清单的节点下；早晨规划日程或临时有空闲，就会查看碎片清单，处理划掉里头的事项。

那些需要记录的讨论内容，需要思考梳理和输出议题或项目，则可以在周单位节点下任意发挥。比如管理组会这个节点下，我一般记录想讨论的议题和纪要。
...
虽然一个层级下节点如此多，早已超过人类大脑最舒适的 4*5 层级，但这不是我的工作台，很少长久于此停留，且可检索关键词，节点再多也无妨。 



至于工作以外的内容，也是按自己的兴趣和输入输出的需求，随心创建节点积累卡片：
...
工作生活免不了各种信息涌入，你是否也有类似烦恼：要记个什么东西时，就打开一个文档或在某某笔记里新建一个页面，过段时间，即使有主题相近的内容，也重复上述动作。久而久之，你会发现他文档越来越多，不仅重复，更悲催的是，大部分并不能马上使用（交付目标用户），且无心整理。

有此困扰，多因把卡片和文件层级的输出混同处理。

需知道，我们要用的，是文件层级的输出——交流协作多基于文件层级，绩效评定也多基于文件以上层级。只要这些层级的产出不乱、鲜有难产，信息再多，你也不会闹心。

回到开头提及的困扰，（团队协作中）多次主题讨论，呈现每次纪要不是目的也不重要，重要的是形成可用的行动方案、可交付用户的成果。
...
WorkFlowy 可以全文检索，检索关键词时可以把当前路径下关键词条目筛选出来，这不特别，所有检索功能都这样。但搭配树形结构，便能挖掘出不少玩法。我最常用的是把常用关键词设定成标签，借助标签提取局部和整体的关系。
 
举个例子：课程组委会每周多个角色协作，每个角色需要了解自己当周什么时候需要做什么，但并非每个角色都需关注所有本周所有角色的安排，那如何从整体快速剥离每个角色所需事务？ WorkFlowy 帮了大忙：设定涉及角色为标签，只要一点击，便可看到这个角色每周所需要跟进事务：
...
再看一例：每周事务繁多，我想了解自己当周在各类事务上所花时间，以判断过去一周的时间分配是否符合角色定位。有 WorkFlowy ，这事很简单——上面提到，我每天在 WorkFlowy 中记录工作日志，在日志里加上事务类别标签及所用工时，周末复盘，点击各标签就可统计各类事务所用时间，相加还可知晓当周自己不是迷迷糊糊、有产出的时长。比如：
...
常看到一些朋友 WorkFlowy 使用网页版，网页不稳定、加载慢，然后跟我说，你推荐那玩意儿不好使啊……
...
不少朋友用 WorkFlowy 时，常进入某个节点，以便保持界面清爽。这就不可避免要退出节点后再进入其它节点，而这其实特别占用工作记忆——你得不停思考自己要找的东西在哪个层级，进入不对还得退出来重新再来，不出五回，就把自己弄晕，尤其节点较深、需要切换的节点较多时。

所以我一般保**持全局视图**。要看某个节点时展开就好，毕竟视窗就那么点，也看不到太多干扰，还方便上下滚动屏幕，查看或拖动节点。

### 期望过奢，用于文件层级



时有朋友吐槽，这玩意儿写文章体验很糟糕啊，写一段一段的文字时体验很差。甚至国内有团队，改良 WorkFlowy 增加写文章、插图等功能开发了一款应用。 

对此我只能笑笑，莫能强求。回到文初，个人输出可以分成三个层级，各层级有各自特点，也有各自匹配的工具，何必用卡片工具输出完整文章？
...
工具是辅助，输出作品才是目标，愿你我都能找到应对各情境的舒服姿势 :)
