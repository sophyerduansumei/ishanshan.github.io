...
要而言之，用 ASCII 码命名，既叫机器可读，人的观感也好不少，一举多得何乐不为？当然，别用汉语拼音哈！
...
如此，想叫文件名在大时间周期内不必变更，有几个要点可循：

1. 命名尽量选用[基本层次范畴(Basic level categories)](https://en.wikipedia.org/wiki/Prototype_theory#Basic_level_categories)词汇，而非下位层次范畴(subordinate categories)词汇。

	这个理解起来较简单，看几个例子你就明白了：
	
	- HbStuGroup VS HbWeChatGroup
	
		之前有同事主动把我们之前每次建学员群的一些固定动作、常用话术整理成了指南，命名 `HbWeChatGroup` 。不久，我们有了自己的 App ，即将不再用 微信 建学员群，这个指南就不得不改文件名或者新建一个文档（但公共界面有内容重复的文档更后患无穷，使不得使不得，新建同类文档为下策）。
		
		如果一开始就以 `HbStuGroup` 甚至 `HbUserGroup` 这类 基本层次范畴 的词汇命名，更换具体群组平台也无妨啦。
...
2. 直接创建可交付的文件，以最终要交付的状态来命名，不必拿过程文档来交流。

	这个不仅是命名更是协作习惯，比较反常识，多数新人甚至有些职场老手也不习惯。我也用了近一年才慢慢掌握。
...
3. 主题内容持续积累到同一指定文档。
	...
	协作越久文档越多，想不信息过载，最好一个场景只保留唯一可用文档，大家都只更新这唯一可用文档。否则内部必会混乱，或维护者不知还有其它版本需要更新，或使用者所用并非新版，烦恼不断。
	
	这种情况下，提前长远思考你创建的文件/文档解决什么问题、怎么命名就更为重要了。
	
	比如你想通过一个页面告诉用户你家 App 怎么使，那这个 HbApp 页面会包含哪些内容，用户常见问题自检解决方案，是放在这个页面，还是另起一个 InfoAppFAQ 的页面？
	
	比如，用户界面已经有了 HbApp 页面告诉用户怎么使用，常见问题如何自检，那内部协作界面，是否还需要一个文档说明内部同事如何处理收到的 App 使用问题？这个文档应该怎么写，怎么命名，才更方便同事长期调用、维护？
	
	再比如，如果已经供内部同事参考的用户的常见问题解决指南，内容越积越多，各类问题是否需要拆分单独文档，如何命名方便长期调用、维护？
...
不管怎样，文档命名规则没有对错之分，若单看某个文件名，队友现在和未来能明白其中所含内容，协作需要时能即时调取，这就是好规则。

祝你远离「各人用词、命名习惯不一样，难以形成共识，徒增团队协作成本」的烦恼（比如俺在[《GitHub 协作五大业余姿势》](http://ishanshan.top/community/HbGitHubCooperate.html) 提到的现象，哈哈）！
